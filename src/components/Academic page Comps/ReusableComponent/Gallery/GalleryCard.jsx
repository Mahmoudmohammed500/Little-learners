...
function GalleryCard(props) {
  ...
  const handleNextClick = () => {
    ...
    if (isTablet) {
      setCurrentImageIndex(
        (prevIndex) => (prevIndex + 2) % props.gallery.length
      );
    } else {
      setCurrentImageIndex(
        (prevIndex) => (prevIndex + 1) % props.gallery.length
      );
    }
  };
  ...
  const handlePrevClick = () => {
    ...
    if (isTablet) {
      setCurrentImageIndex(
        (prevIndex) =>
          (prevIndex - 2 + props.gallery.length) % props.gallery.length
      );
    } else {
      setCurrentImageIndex(
        (prevIndex) =>
      ...
      <div className={styles.ImageContainer}>
        {isMobile ? (
          ...
          props.gallery.map((Galleryimage, index) => (
            <CardImage key={index} imgsrc={Galleryimage} />
          ))
        )}
      </div>
